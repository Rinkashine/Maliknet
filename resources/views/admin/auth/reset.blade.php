    ...
    <div class="block xl:grid grid-cols-2 gap-4">
        <div class="hidden xl:flex flex-col min-h-screen">
            <a href="#" class="-intro-x flex items-center pt-5">
                <img alt="{{ config('app.name') }} Logo" class="w-6" src="{{asset('icons/log.png')}}">
                <span class="text-white text-lg ml-3">{{ config('app.name') }}</span>
            </a>
            <div class="my-auto">
                <img alt="{{ config('app.name') }} Image" class="-intro-x w-1/2 -mt-16" src="{{ asset('dist/images/undraw_inspiration.svg') }}">
                <div class="-intro-x text-white font-medium text-4xl leading-tight mt-10">Forgotten Password?</div>
                <div class="-intro-x mt-5 text-lg text-white text-opacity-70 dark:text-slate-400">No Worries {{ config('app.name') }} got you</div>
            </div>
        </div>
            ...
            <div class="my-auto mx-auto xl:ml-20 bg-white dark:bg-darkmode-600 xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none w-full sm:w-3/4 lg:w-2/4 xl:w-auto">
                ...
                <form action="{{Route('reset.store')}}" method="POST">
                    ...
                    <div class="intro-x mt-2 text-slate-400 dark:text-slate-400 xl:hidden text-center">
                        Forgotten Password? No Worries {{ config('app.name') }} got you
                    </div>

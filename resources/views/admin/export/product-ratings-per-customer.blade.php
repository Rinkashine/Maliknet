...
<html>
<head>
    <title>{{ $product_name }} Ratings</title>
</head>
<style type="text/css">
    @page {
...
<body class="pt-x-50">
    ...
    <main>
        <div class="w-100" style="margin-bottom:10px">
            <div>
                <h3 class="text-center header-title">{{ $product_name }} Ratings </h3>
        ...
        <div>
            <table class="table w-100">
                <thead>
                    <tr class="text-white">
                        <th class="w-50 th-color-dark">Order Id</th>
                        <th class="w-50 th-color-dark">Name</th>
                        <th class="w-50 th-color-dark">Rate</th>
                    </tr>
                </thead>
               <tbody>
                    @foreach($reviews as $review)
                        <tr class="stripe">
                            <td class="text-center">{{ $review->customer_order_id, }}</td>
                            <td class="text-center" style="font-family: DejaVu Sans; font-size: 0.8rem;">{{ $review->name }}</td>
                            <td class="text-center" style="font-family: DejaVu Sans; font-size: 0.8rem;">{{ number_format($review->rate) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
